    ...
    try:
        with st.spinner('正在分析數據中...'):
            ...
            if not 融資券總表.empty:
                ...
                if len(融資券總表) >= 2:
                    前日總表 = 融資券總表.iloc[-2]
                    大盤融資增減 = (int(最新總表.get("TodayBalance", 0)) - int(最新總表.get("TodayBalance", 0))) // 1000 # Corrected column name
                    大盤融券增減 = (int(最新總表.get("TodayBalance", 0)) - int(最新總表.get("TodayBalance", 0))) // 1000 # Corrected column name
