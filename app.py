...
st.markdown("""
    ...
    .data-label { font-size: 18px; font-weight: bold; color: #FFFFFF; margin-top: 10px; }
    .val-pos { font-size: 22px; font-weight: bold; color: #ff4b4b; } 
    .val-neg { font-size: 22px; font-weight: bold; color: #00c853; } 
...
開始日期 = st.sidebar.date_input("開始日期", value=pd.to_datetime("2026-03-01"))
結束日期 = st.sidebar.date_input("結束日期", value=datetime.now().date() - timedelta(days=1))
執行診斷 = st.sidebar.button("開始執行診斷")

if not 執行診斷:
    st.title("🚀 台股籌碼智慧診斷系統")
    st.info("👈 請在左側設定後按下「開始執行診斷」。")
else:
    try:
        with st.spinner('正在分析數據中...'):
            ...
            # 基本面 (最近一年)
            base_date = (datetime.now() - timedelta(days=500)).strftime('%Y-%m-%d')
        ...
        if not 法人資料.empty:
            ...
            elif f_buy < 0 and t_buy > 0: st.write("⚠️ **[土洋對作]**：投信力挺但外資在倒貨，需觀察 5MA 支撐是否守住。")
            elif f_buy > 0 and t_buy < 0: st.write("⚠️ **[外熱內冷]**：外資回頭補貨，但內資投信先行獲利了結。")
        ...
        # --- 4. 三大分頁圖表 ---
        st.markdown("---")
        tab1, tab2, tab3 = st.tabs(["價格趨勢", "法人五日趨勢", "基本面：營收與毛利趨勢"])
        ...
        with tab2:
            if not 法人資料.empty:
                ...
                ax2.set_title("近五日法人買賣超趨勢 (張)")
                st.pyplot(fig2)

        with tab3:
            if not 營收資料.empty:
                fig3, ax3 = plt.subplots(figsize=(12, 6))
                ax3.bar(pd.to_datetime(營收資料['date']), 營收資料['revenue']//1000000, color='#add8e6', label='月營收(百萬)')
                if not 損益表.empty:
                    ax4 = ax3.twinx()
                    gpm = 損益表[損益表['type'] == 'GrossProfitMargin'].tail(4)
                    ax4.plot(pd.to_datetime(gpm['date']), gpm['value'], color='red', marker='o', label='毛利率%')
                st.pyplot(fig3)
            else: st.warning("無法取得基本面資料。")
        ...
        if "GEMINI_API_KEY" in st.secrets:
            try:
                ...
                prompt = f"分析台股{股票代號}{股名}: 價格{最新股價}, 外資{外資:+,d}, RSI{最新RSI:.1f}。請給予專業投資建議。"
                response = model.generate_content(prompt)
                st.info(f"🤖 AI 投資顧問分析：\n\n{response.text}")
            except Exception as e:
                st.warning(f"🕒 AI 顧問目前忙碌中 (錯誤: {str(e)})，請參考上方深度文字分析。")
