 ...
 # 抓取法人買賣超資料 (取最近 30 天以確保有足夠五日數據)
        ...
        with tab1:
            ...
            if not plot_sbl.empty:
                ...
                with t_col2:
                    st.write("**【籌碼空方動向】**")
                    if 連續回補 >= 3:
                        st.write(f"✅ **空頭認賠回補**：借券已連續 `{連續回補}` 天回補。這種『還券大於賣出』的現象若配合股價止跌，通常是法人空單撤退、底部浮現的訊號。")
                    elif 借券賣出 > 今日還券:
                        st.write(f"❌ **空方持續加壓**：今日借券賣出張數仍大於還券，且總餘額維持在 `{最新借券餘額:,.0f}` 張高位。顯示空頭勢力尚未鬆手，對股價形成實質壓力。")
                    else:
                        st.write(f"● **鎖單觀察**：借券餘額變動不大。目前空頭處於「鎖單」狀態，正在等待市場下一個變盤訊號。")

                    st.write(f"● **追價力道**：今日成交張數為 `{今日張數:,.0f}` 張，與 5MA 均量相比為 `{(今日張數/今日5MA量 if 今日5MA量>0 else 1):.2f}` 倍。{'量能不足，推升力道受限。' if 今日張數 < 今日5MA量 else '量能升溫，市場參與度提高。'}")
        ...
        with diag_col1:
            ...
            if sbl_val > 0:
                ...
                st.write(f"● **[籌碼鎖定]**: 借券餘額 `{sbl_val:,.0f}` 張。{sbl_desc}")
            else:
        ...
        st.markdown("---")
        # 根據建議給予不同顏色的提醒
        if "加碼" in 建議 or "買進" in 建議:
            st.info(f"🔍 **操作核心建議：{建議}**\n\n**分析理由**：{理由}\n\n*當前籌碼集中度：`{籌碼集中度:.2f}%`。*")
        elif "獲利了結" in 建議 or "減碼" in 建議:
            st.warning(f"🔍 **操作核心建議：{建議}**\n\n**分析理由**：{理由}\n\n*目前 RSI 已達 `{最新RSI:.1f}`，請注意回測風險。*")
        else:
            st.write(f"🔍 **操作核心建議：{建議}**\n\n**分析理由**：{理由}")
