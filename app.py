...
if not 執行診斷:
    ...
    # 預設變數初始化 (防止報錯)
    最新股價, 最新5MA, 最新RSI, 籌碼集中度 = 0, 0, 0, 0
    最新營收, latest_gp, 最新借券餘額 = 0, 0, 0
    latest_day_dict = {}
    ...
    try:
        ...
        if not 借券資料.empty:
            ax2.bar(pd.to_datetime(借券資料['date']), (借券資料['SBLShortSalesReturns'] - 借券資料['SBLShortSalesShortSales'])//1000, color='green', alpha=0.5, label='淨回補')
